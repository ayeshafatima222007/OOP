    ...
    internal class StudentUI
    {
        public static StudentBL TakeInputForStudent()
        {
            ...
            while (true)
            {
                ...
                if (float.TryParse(Console.ReadLine(), out fsc)) break;
            ...
            while (true)
            {
                ...
                if (float.TryParse(Console.ReadLine(), out ecat)) break;
                Console.WriteLine("Enter Valid Marks!!");
            }
            ...
            while (true)
            {
                ...
                if (int.TryParse(Console.ReadLine(), out pref)) break;
        ...
        public static void RegisterSubjects(StudentBL s)
        {
            if (s != null)
            {
                List<SubjectBL> subjects = SubjectDL.GetSubjectsByDegree(Convert.ToString(s.GetRegDegree()?.GetTitle()));     // Load subjects from database instead of in-memory
                ...
                // ✅ Find subject from database list
                SubjectBL selectedSubject = SubjectDL.FindByCode(subjects, code);
